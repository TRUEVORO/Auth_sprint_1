...
- [Auth_sprint_1](https://github.com/TRUEVORO/Auth_sprint_1)
...
### 1. В .env задать REDIS_HOST=localhost (спасибо flask_jwt_extended, более изящного решения, к сожалению не нашел)
### 2. Запустить тесты
```
make tests
```
### PS:
Конструкция с downgrade по завершении тестов подпортилась после добавление теста на auth_history. Он выполняется, но вот downgrade не позволяет завершить процесс (логов никаких нет), если комментить это тест, все работает как надо
